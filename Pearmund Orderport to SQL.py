...
csv_file_input = 'sales-master-feb24-pc.csv'
...
campaign_files = [
    {
        'name': '02.05.24',
        'opened': "contact_export_pearmund Opened.csv",
        'sent': 'contact_export_pearmund Sent.csv',
        'clicked': 'contact_export_pearmund Clicked.csv'
    }
] 
...
member_startdates_actual = pd.read_excel('WineClubStartDatesPearmund.xlsx')
...
df.to_sql('orderport_pc', con=engine, if_exists='replace', index=False)
all_campaigns_data.to_sql('campaigns_pc', con=engine, if_exists='replace', index=False)
df_merged.to_sql('visits_pc', con=engine, if_exists='replace', index=False)
